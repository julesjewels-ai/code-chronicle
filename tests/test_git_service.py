...
class TestLocalGitService(unittest.TestCase):
    ...
    def test_get_commit_history_success(self, mock_popen):
        ...
        # process.stdout needs to be iterable.
        # Since we iterate over it in the loop.
        ...
        self.assertEqual(
            args[0][:5], ["git", "-C", "/tmp/repo", "log", "-n"]
        )
        self.assertEqual(args[0][6], "--pretty=format:%h|%s")
        self.assertEqual(kwargs['stdout'], subprocess.PIPE)
        self.assertEqual(kwargs['text'], True)
        # bufsize is default (-1), so it might not be in kwargs or be -1
        # depending on impl. We removed explicitly setting it to 1.
