...
def test_main_defaults(mock_dependencies):
    ...
    deps['parse_args'].return_value = argparse.Namespace(path='.', limit=5, format='console', api_key=None, model='gpt-3.5-turbo')
...
def test_main_custom_args(mock_dependencies):
    deps = mock_dependencies
    deps['parse_args'].return_value = argparse.Namespace(path='/custom/repo', limit=10, format='markdown', api_key=None, model='gpt-3.5-turbo')
...
def test_main_with_api_key(mock_dependencies):
    deps = mock_dependencies
    deps['parse_args'].return_value = argparse.Namespace(path='.', limit=5, format='console', api_key='test-key', model='gpt-4')

    main()

    deps['git'].assert_called_once_with(".")
    deps['llm'].assert_not_called()
    deps['openai_llm'].assert_called_once_with(api_key='test-key', model='gpt-4')
    deps['generator'].return_value.generate.assert_called_once_with(limit=5)

def test_main_invalid_path(mock_dependencies):
    deps = mock_dependencies
    deps['parse_args'].return_value = argparse.Namespace(path='/invalid/path', limit=5, format='console', api_key=None, model='gpt-3.5-turbo')
