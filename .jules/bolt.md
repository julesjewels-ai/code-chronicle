...
## 2026-02-12 - [Dataclass Import Cost] **Learning:** Importing 'dataclasses' (and its dependencies like 'inspect') costs ~50ms+ at startup. For simple DTOs in CLI tools, manual classes with '__slots__' are significantly faster to import (~10x) and maintain similar memory efficiency. **Action:** Avoid 'dataclasses' in critical startup paths for simple data models.
