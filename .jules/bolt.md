...
## 2026-02-12 - [Git Log Parsing Optimization] **Learning:** Inlining parsing logic and using `tformat` with null delimiters (`%x00`) provided robust handling of special characters and a ~8% performance gain over a helper function, while `tformat` ensured consistent trailing newlines unlike `format`. **Action:** Use inlining for tight loop parsing and `tformat` for line-oriented git output.
