...
## 2026-10-24 - [Concurrent Futures vs Threading Import] **Learning:** Even lazily importing `concurrent.futures` incurs a heavy cost (~70-80ms due to `logging` dependency) which dominates execution time for short-lived CLI tasks. Replacing it with manual `threading` + `queue` (import cost ~20ms) yielded a ~15x speedup (253ms -> 17ms) for small workloads. **Action:** For performance-critical CLI tools with short tasks, prefer lightweight `threading`/`queue` over `concurrent.futures`.
