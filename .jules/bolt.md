...
## 2026-02-14 - [ThreadPoolExecutor Sizing] **Learning:** Explicitly setting `max_workers` to match known small workload sizes prevents `ThreadPoolExecutor` from over-provisioning resources or performing unnecessary CPU count checks (micro-optimization). **Action:** Clamp `max_workers` when workload size is known and small.
